...
exports.recordVisit = asyncHandeler(async (req, res, next) => {
    ...
    if (cookie == null) {
        website.visitors++;
        website.visitorsHistory = addView(website.visitorsHistory ?? [], website.visitorsHistory.length == 0 ? website.visitors : 1);

        res.setHeader('Set-Cookie', `${configs.cookieName}=true; Secure=true; Path=/; SameSite=None; HttpOnly`);
    }
    website.visits++;
    website.visitsHistory = addView(website.visitsHistory ?? [], website.visitsHistory.length == 0 ? website.visits : 1);
